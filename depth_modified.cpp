...
static SE_ITR_Behaviour glb_itr_traversal = SE_ITRBEH_RESOLVE; //ITR references are resolved automatically as they are encountered

//This data type specifies the desired traversal for an iterator, which is used to iterate through a list of objects
//  SE_ITR_Behaviour
...
{
    seWorkspace     workspace; //Open a transmittal
    ...
    SE_SetOutFilePtr(stdout); // Sets the FILE pointer to which output is directed.
    SE_SetErrorFilePtr(stderr); //Sets the FILE pointer to which error messages are directed, if error messages are requested.
    glb_out_file_ptr = stdout;  //The destination output for all non-error statements.
    glb_err_file_ptr = stderr;  // The destination output for all error statements.
    
    
    
    //ADDED BY ELISA
    //static SE_Search_Filter = filter;
    
    static SRM_Long_Float latitude1  = 98, latitude2  = 110;
    static SRM_Long_Float longitude1 = -10, longitude2 = 15;
    ...
    workspace.setITRBehaviour(glb_itr_traversal); //Specifies the desired traversal for an iterator or function when an ITR reference is encountered.

    try
    {
        workspace.openTransmittalByFile(glb_file_name.c_str(), transmittal); // Open the transmittal
    }
    ...
    {
        transmittal.getRootObject(start_obj); //Get the root object of a transmittal (root of the DRM hierarchy
    ...
    //    SE_InitializeAggregateIterator(start_obj, seObject::seObjectNull, false);
